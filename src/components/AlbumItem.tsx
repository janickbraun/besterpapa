...
export default function AlbumItem({ title, imageUrl, index }: AlbumItemProps) {
  ...
  return (
    <>
      <motion.div
        ...
        className={`flex flex-col ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'} items-center gap-8 md:gap-12 w-full max-w-5xl mx-auto`}
      >
        {/* Image Container */}
        <div className={`w-full md:w-1/2 flex justify-center ${isEven ? 'md:justify-end' : 'md:justify-start'}`}>
          <motion.div 
            ...
            className="relative bg-warm-white p-4 rounded-xl scrapbook-shadow w-full max-w-[400px] cursor-zoom-in"
        ...
        {/* Text Container */}
        <div className={`w-full md:w-1/2 flex flex-col items-center ${isEven ? 'md:items-start md:text-left' : 'md:items-end md:text-right'} text-center space-y-4`}>
            ...
            >
              ...
              >
                <img
                  src={imageUrl}
                  alt={title}
                  className="max-w-full max-h-[85vh] md:max-h-[90vh] object-contain rounded-lg shadow-2xl w-auto h-auto"
                />
              </motion.div>
            </motion.div>
          )}
